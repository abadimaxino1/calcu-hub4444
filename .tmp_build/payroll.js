...
function calcPayroll(args) {
  ...
  const housingForBase = housingFromBasic(mode === "net2gross" && grossOverride == null ? Math.max(assumedBasicForN2G, 1) : basic);
  const baseGosi = Math.min(basic + housingForBase, GOSI_CAP);
  const empBase = insBase === "gosi" ? baseGosi : insBase === "basic" ? basic : gross;
  ...
  if (insBase === "gross" && gross > 0 && insuranceEmployee > 0) {
    const parts = { basic, housing: housingFromBasic(basic), transport, other: otherAllow };
